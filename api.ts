...
function checkStatus(response: Response) {
    if (!response.ok) {
        ...
        throw new Error(`Status Code: ${response.status} ${response.url} ${(response as (Response & { error: string })).error}`)
    }
    return response
}
...
export async function fetchChromeUrl(branchPosition: number, osSettings: IOSSettings): Promise<string | undefined> {
    ...
    const chromeMetadataResponse = await fetch(snapshotUrl)
        .then(checkStatus)
        .then(toJson) as IMetadataResponse
...
export async function fetchChromeZipFile(url: string): Promise<Response> {
...
export async function fetchReleases(platform: Platform, channel: Channel, amount = 100): Promise<ApiRelease[]> {
    return fetch(`https://chromiumdash.appspot.com/fetch_releases?channel=${channel}&platform=${platform}&num=${amount}&offset=0`)
        .then(checkStatus)
        .then(toJson) as Promise<ApiRelease[]>
}
